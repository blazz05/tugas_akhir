...
@section('title', 'Form Edit Tasmi')
@section('content_header')
    <h1>Data Tasmi</h1>
@stop
@section('content')
 <!-- Ini Konten Form Edit Tasmi  -->
 @php
$rs1 = App\Models\Mahasantri::all();
@endphp
@foreach($data as $rs)
    <form method="POST" action="{{ route('tasmi.update',$rs->id) }}">
@csrf 
...
<!--  method put digunakan untuk meletakkan data yang akan diubah disetiap element form edit setoran  -->
    <div class="form-group">
        ...
        <select class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"" name="setoran_mahasantri_id">
            ...
            @foreach($rs1 as $nama)
                @php
                    $sel1 = ($nama->id == $rs->setoran_mahasantri_id) ? 'selected' : '';
                @endphp
    ...
    <div class="mb-3">
        ...
        <input type="text" name="setoran_id" value="{{ old('setoran_id', $rs->setoran_id) }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required />
    </div>
    <div class="form-group">
        ...
        <select class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" name="keterangan" id="keterangan">
            <option value="ممتاز" {{ (old('keterangan') ?? $rs->keterangan) == 'ممتاز' ? 'selected' : '' }}>ممتاز</option>
            <option value="جيد جدا" {{ (old('keterangan') ?? $rs->keterangan) == 'جيد جدا' ? 'selected' : '' }}>جيد جدا</option>
            <option value="جيد" {{ (old('keterangan') ?? $rs->keterangan) == 'جيد' ? 'selected' : '' }}>جيد</option>
            <option value="مقبول" {{ (old('keterangan') ?? $rs->keterangan) == 'مقبول' ? 'selected' : '' }}>مقبول</option>
            <option value="مردود" {{ (old('keterangan') ?? $rs->keterangan) == 'مردود' ? 'selected' : '' }}>مردود</option>
        </select>
    </div>
    ...
    <a class="bg-blue-500 text-white font-bold py-2 px-3 rounded float-right" href="/tasmi" role="button">Back&nbsp;&nbsp;<i class="fa fa-arrow-left"></i></a>
</form>
@endforeach
@stop
