...
@php
$ar_judul = ['No','Nim','Nama','Ujian Tahfizh', 'Keterangan','Action'];
$no = 1; 
@endphp
     ...
     <!-- /.card -->
        <div class="card">
            <div class="card-header">
                <a class="btn btn-primary btn-md bg-blue-500" href="{{ route ('mahasantri.create') }}" role="button">Tambah Mahasantri  <i class="fa fa-plus"></i></a>
            </div>
            <!-- /.card-header -->
            <div class="card-body">
                ...
                <table id="example1" class="table table-bordered table-striped">
                    ...
                    @foreach($ar_mahasantri as $mahasantri)
                        <tr class="hover:bg-gray-100 dark:hover:bg-neutral-700">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-neutral-200">{{ $mahasantri->nim }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-neutral-200">{{ $mahasantri->nama }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-neutral-200">{{ $mahasantri->lulus }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-neutral-200">{{ $mahasantri->keterangan }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-neutral-200">
                                <form method="POST" action="{{ route('mahasantri.destroy', $mahasantri->id) }}">
                                    ...
                                    <a class="btn btn-info" href="{{ route('mahasantri.show', $mahasantri->id) }}"><i class="fa fa-eye"></i></a>
                                    <a class="btn btn-success" href="{{ route('mahasantri.edit', $mahasantri->id) }}"><i class="fa fa-pen"></i></a>
